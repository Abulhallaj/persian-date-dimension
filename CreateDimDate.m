let fxDateShamsi = (Start.Date as date, optional End.Date as date) as table => let
    ...
    DateKey = Table.AddColumn(DateFormat, "DateKey", each Date.ToText([Date],"yyyyMMdd")),
    ShamsiDate = Table.AddColumn(DateKey, "Shamsi Date", each Date.ToText([Date],"yyyy-MM-dd","fa-IR")),
